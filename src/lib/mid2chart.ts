...
export function mid2Chart(
  ...
  function writeSync(track: MidiEvent[]): void {
    Sync += "[SyncTrack]\n{\n";
    let tick = 0;
    let event: MidiEvent;
  ...
  function writeNoteSection(track: MidiEvent[], sec: number): void {
  ...
  function writeEventLine(sec: number, tick: number, event: string): void {
    let e = event;
    if (sec == 4) {
      e = '"' + event + '"';
    }
